...
Defines the app's global configuration: a persisted, observable settings model and the native `Settings` scene that hosts it — the refresh-cadence preset, the menu bar live-indicator and team-logo options — with defaults and persistence for an accessory (Dock-less) menu bar app.
