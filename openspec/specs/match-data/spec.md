...
The system SHALL access all football match data through a `MatchDataProvider` abstraction, so the concrete data source can be replaced or supplemented without changing the store or views. The provider SHALL accept a set of leagues and return the matches for those leagues.
...
#### Scenario: Provider returns matches for requested leagues
- **WHEN** the provider is asked for matches for a set of leagues
- **THEN** it returns the matches belonging to those leagues and no others

### Requirement: ESPN provider implementation

The system SHALL provide an ESPN-backed implementation of `MatchDataProvider` that reads the ESPN scoreboard endpoint `site.api.espn.com/apis/site/v2/sports/soccer/{slug}/scoreboard` for each requested league using `async/await` and `URLSession`, with no API key.

#### Scenario: Fetch a league scoreboard
- **WHEN** the ESPN provider fetches a league identified by ESPN slug `fifa.world`
- **THEN** it requests the scoreboard endpoint for that slug
- **AND** it decodes the response into the app's match models

#### Scenario: Multiple leagues fetched per request
- **WHEN** the provider is asked for matches across several leagues
- **THEN** it issues one scoreboard request per league
...
The system SHALL represent match data as Codable value types: `League`, `Match`, `Team`, `Score`, and a `MatchState`. `MatchState` SHALL distinguish upcoming, in-progress, and finished matches, derived from ESPN's per-event `state` values `pre`, `in`, and `post`.
